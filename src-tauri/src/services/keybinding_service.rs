...
impl KeybindingService {
    ...
    pub fn import_keybindings(&self, json_string: &str) -> Result<()> {
        ...
        let value: serde_json::Value = serde_json::from_str(json_string)
            .map_err(|e| AppError::InvalidInput(format!("Failed to parse JSON: {}", e)))?;

        // 验证版本
        let version = value.get("version")
            .and_then(|v| v.as_str())
            .ok_or(AppError::InvalidInput("Missing version field".to_string()))?;

        if version != "1.0" {
            return Err(AppError::InvalidInput(format!("Unsupported version: {}", version)));
        }
        ...
        let keybindings: HashMap<String, crate::models::KeyCombination> =
            serde_json::from_value(
                value.get("keybindings")
                    .ok_or(AppError::InvalidInput("Missing keybindings field".to_string()))?
                    .clone()
            ).map_err(|e| AppError::InvalidInput(format!("Failed to parse keybindings: {}", e)))?;

        // 解析 presets（可选）
        let presets = if let Some(presets_value) = value.get("presets") {
            serde_json::from_value(presets_value.clone())
                .map_err(|e| AppError::InvalidInput(format!("Failed to parse presets: {}", e)))?
        } else {
            vec![]
        };

        self.save_keybindings(keybindings, presets)?;
        log::info!("Keybindings imported successfully");
        Ok(())
    }
